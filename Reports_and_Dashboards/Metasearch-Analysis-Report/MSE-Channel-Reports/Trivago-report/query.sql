...
SUM(trivago_clicks) as Clicks,
SUM(trivago_cost) as Cost
...
Markup_Amount,
Markup_Percent,
ROUND(trivago_cost.Cost,0) as Cost,
ROUND((Commission-GBV*0.03),0) as Margin,
ROUND((Commission-trivago_cost.Cost-GBV*0.03),0) as PC3
FROM trivago_data
LEFT JOIN trivago_clicks ON 1=1
AND trivago_data.Date1 = trivago_clicks.Date2
LEFT JOIN trivago_cost ON 1=1
AND trivago_data.Date1 = trivago_cost.Date3
ORDER BY 1 DESC
