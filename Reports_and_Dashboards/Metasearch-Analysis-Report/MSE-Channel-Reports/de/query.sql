...
case when ROUND(SUM(results),0) > 0 then ROUND(SUM(results),0) else 0 end  as Total_Results
FROM
(
...
(
  ...
  and d58_user_base_country_name = 'Germany'
  GROUP BY 1
  HAVING bookings > 0
  ORDER BY 2 DESC
),
...
select
  ...
  Commission,
  cost,
--  case when (Commission - cost < 0) then -(Commission - cost) else 0 end as results
