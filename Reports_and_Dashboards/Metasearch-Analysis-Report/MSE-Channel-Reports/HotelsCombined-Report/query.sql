...
WHERE 1=1
AND lower(d180_order_referral_source_code) = 'hotelscombined'
...
WHERE 1=1
AND consumer_code ='hotelscombined'
...
SUM(hotelscombined_cost) as Cost
...
Markup_Amount,
Markup_Percent,
ROUND(hc_cost.cost,0) as Cost,
ROUND((Commission-GBV*0.03),0) as Margin,
ROUND((Commission-hc_cost.Cost-GBV*0.03),0) as PC3
FROM hc_data
LEFT JOIN hc_clicks ON 1=1
AND hc_data.Date1 = hc_clicks.Date2
LEFT JOIN hc_cost ON 1=1
AND hc_data.Date1 = hc_cost.Date3
ORDER BY 1 DESC
